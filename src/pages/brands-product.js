...
import { BRANDS_PRODUCT } from "../service/endpoints";
...
import ModalBrand from "src/sections/product/brand/modal-brand";
...
const Page = () => {
  ...
  const columns = [
    { field: "prBrId", headerName: "ID", type: "number", width: 100 },
    { field: "prBrName", headerName: "NAME", width: 200 },
    {
      field: "options",
      headerName: "OPTIONS",
      width: 200,
  ...
  const getData = async () => {
    var response = await getElements(`${BRANDS_PRODUCT.list}?page=${0}&size=${500}`, {
      jwt: `${user.id}`,
    });
  ...
  const update = async (obj) => {
    var response = await putElements(
      `${BRANDS_PRODUCT.update.replace("{id}", obj.prBrId)}`,
      {
        "Content-Type": "application/json",
        jwt: `${user.id}`,
      },
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        var response = await deleteElements(
          `${BRANDS_PRODUCT.delete.replace("{id}", obj.prBrId)}`,
          {
            "Content-Type": "application/json",
            jwt: `${user.id}`,
          }
  ...
  return (
    <>
      <Head>
        <title>Brands Product | Devias Kit</title>
      </Head>
      <Box
        component="main"
        sx={{
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            <Stack direction="row" justifyContent="space-between" spacing={4}>
              <Stack spacing={1}>
                <Typography variant="h4">Brands Product</Typography>
              </Stack>
              <div>
                <ButtonCustom
                  openModal={() => {
